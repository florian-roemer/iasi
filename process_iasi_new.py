...
def save_flux(specflux, nobs, frac, year, month, orbit, domain):
    ...
    ## CHANGED!!!
    domain = f'{domain[0]}/{domain[1]}/{domain[2]}'
    ...
    np.save('../data/IASI/test_new/{}/{}/{}/frac_{}'
            .format(domain, year, month, orbit),
...
def create_mask(lat, land_frac, cloud_frac, domain):
    ...
    size = np.count_nonzero(mask)/mask.size
    ...
    for dom1 in ['global', 'tropics']:
        for dom2 in ['all-sky', 'clear-sky']:
            for dom3 in ['whole', 'ocean']:
                ...
                mask, size = create_mask(lat, land_frac, cloud_frac, domain)
                specflux, nobs, frac = process_data(radiance, angle, mask,
                                                    domain, orbit)

    # return CHANGED
    return mask, nobs, frac
...
#    day = sys.argv[3]
    ...
    year, month, day = '2017', '03', '09'

    # name of path CHANGED
    p = f'/work/um0878/data/iasi/iasi-l1/reprocessed/m02/{year}/{month}/{day}/'

    for FILE in np.sort(glob.glob(p + 'IASI*', recursive=False))[10:11]:
        print(FILE[74:105])
        try:
            # paremter return CHANGED
            mask, nobs, frac = main(FILE)
        except:
